...
EOF
    ...
    tmpfile_plugins=$(mktemp)
    printf "%s\n" "$CUSTOM_PLUGINS" > "$tmpfile_plugins"
    ...
    sed -i "/-- NOTE: Plugins can specify dependencies./r $tmpfile_plugins" "$INIT_LUA"
...
EOF
    ...
    # Write the block to a temporary file
    tmpfile_options=$(mktemp)
    printf "%s\n" "$CUSTOM_OPTIONS" > "$tmpfile_options"

    einfo "Adding custom vim options..."
    sed -i "/vim\.o\.confirm = true/r $tmpfile_options" "$INIT_LUA"
...
EOF
    ...
    # Write the block to a temporary file
    tmpfile_keymaps=$(mktemp)
    printf "%s\n" "$CUSTOM_KEYMAPS" > "$tmpfile_keymaps"
    ...
    sed -i "/-- NOTE: Some terminals have colliding keymaps or are not able to send distinct keycodes/r $tmpfile_keymaps" "$INIT_LUA"
